...
func apply_effects(boxer : Boxer, enemy : Enemy) -> void:
	...
	if boxer_health_damage != 0:
		...
		boxer_damage_effect.execute_effect(boxer.boxer_stats, enemy.enemy_stats)
	...
	if enemy_health_damage != 0:
		...
		enemy_damage_effect.execute_effect(enemy.enemy_stats, boxer.boxer_stats)
	...
	if boxer_stamina_damage != 0:
		...
		boxer_stamina_effect.execute_effect(boxer.boxer_stats, enemy.enemy_stats)
